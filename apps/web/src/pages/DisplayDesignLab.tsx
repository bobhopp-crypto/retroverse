        ...
        <div style={{ display: 'grid', gap: 8 }}>
          <div className="inline-actions" style={{ flexWrap: 'wrap' }}>
            ...
            {YEAR_OPTIONS.map((option) => (
              <button key={option} type="button" onClick={() => setYear(option)} style={{ background: option === year ? 'var(--rv-btn-primary-bg)' : 'var(--rv-btn-secondary-bg)', color: option === year ? 'var(--rv-btn-primary-fg)' : 'var(--rv-btn-secondary-fg)' }}>
                {option}
              </button>
            ))}
          </div>
          ...
          <div className="inline-actions" style={{ flexWrap: 'wrap' }}>
            ...
            {TIER_OPTIONS.map((option) => (
              <button key={option} type="button" onClick={() => setTier(option)} style={{ background: option === tier ? 'var(--rv-btn-primary-bg)' : 'var(--rv-btn-secondary-bg)', color: option === tier ? 'var(--rv-btn-primary-fg)' : 'var(--rv-btn-secondary-fg)' }}>
                {option}
              </button>
            ))}
          </div>
