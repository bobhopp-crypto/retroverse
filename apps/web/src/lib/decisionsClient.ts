...
export const fetchLatestMatches = async (): Promise<MatchFile> => {
  const res = await fetch(`${PIPELINE_API_BASE}/matching/latest`)
  if (!res.ok) throw new Error(`Failed to load matches (${res.status})`)
  return res.json()
}

export const fetchDecisionLedger = async () => {
  const res = await fetch(`${PIPELINE_API_BASE}/decisions`)
...
export const updateDecision = async (videoId: string, record: DecisionRecord) => {
  const res = await fetch(`${PIPELINE_API_BASE}/decisions/${encodeURIComponent(videoId)}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
export const deleteDecision = async (videoId: string) => {
  const res = await fetch(`${PIPELINE_API_BASE}/decisions/${encodeURIComponent(videoId)}`, { method: 'DELETE' })
  if (!res.ok) {
    const err = await res.json().catch(() => ({}))
...
export const acceptDecisions = async (records: DecisionRecord[]) => {
  const res = await fetch(`${PIPELINE_API_BASE}/decisions/accept`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(records),
  })
...
export const fetchMatchingOverview = async () => {
  const res = await fetch(`${PIPELINE_API_BASE}/matching/overview`)
...
export const checkApiHealth = async (): Promise<boolean> => {
  try {
    const res = await fetch(`${PIPELINE_API_BASE}/health`)
    return res.ok
  } catch {
    return false
  }
...
export const searchBillboardSongs = async (query: string, limit = 15) => {
  const res = await fetch(
    `${PIPELINE_API_BASE}/search/billboard?query=${encodeURIComponent(query)}&limit=${limit}`,
  )
  if (!res.ok) {
    const err = await res.json().catch(() => ({}))
