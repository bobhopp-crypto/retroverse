...
export const VIDEO_INDEX_DECADES = ['1920s', '1930s', '1940s', '1950s', '1960s', '1970s', '1980s', '1990s', '2000s', '2010s', '2020s'] as const

const decadeCache = new Map<string, VideoRecord[]>()
const decadeInFlight = new Map<string, Promise<VideoRecord[]>>()
let allDecadesInFlight: Promise<VideoRecord[]> | null = null

const mergeCachedDecades = (): VideoRecord[] => {
  const merged: VideoRecord[] = []
  const orderedKeys = [...VIDEO_INDEX_DECADES, ...[...decadeCache.keys()].filter((key) => !VIDEO_INDEX_DECADES.includes(key as (typeof VIDEO_INDEX_DECADES)[number]))]
  const seen = new Set<string>()

  for (const key of orderedKeys) {
    if (seen.has(key)) continue
    seen.add(key)
    const rows = decadeCache.get(key)
...
const fetchAndParseVideoIndex = async (url: string): Promise<VideoRecord[]> => {
    ...
    Array.isArray(body)
      ...
      : Array.isArray(body?.items)
        ...
        : Array.isArray(body?.videos)
          ? body.videos
          : []
  ) as RawVideoIndexItem[]
  return parseVideoIndexRows(items)
}

const loadVideoIndexDecade = async (decade: string): Promise<VideoRecord[]> => {
  const key = decade.trim().toLowerCase()
  if (!key) return []

  const cached = decadeCache.get(key)
  ...
  decadeInFlight.set(key, promise)
  try {
    return await promise
  } finally {
    decadeInFlight.delete(key)
  }
}

export const loadVideoIndex = async (decade?: string): Promise<VideoRecord[]> => {
  if (typeof decade === 'string' && decade.trim()) {
    return loadVideoIndexDecade(decade)
  }

  if (allDecadesInFlight) return allDecadesInFlight

  const missingDecades = VIDEO_INDEX_DECADES.filter((label) => !decadeCache.has(label))
  if (missingDecades.length === 0) return mergeCachedDecades()

  allDecadesInFlight = (async () => {
    for (const label of missingDecades) {
      await loadVideoIndexDecade(label)
    }
    return mergeCachedDecades()
  })()

  try {
    return await allDecadesInFlight
  } finally {
    allDecadesInFlight = null
  }
}
